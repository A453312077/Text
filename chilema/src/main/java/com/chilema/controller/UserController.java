...
public class UserController {
	...
	public String home(HttpServletRequest req){
		...
		req.setAttribute("shop",findShopssss.getDataAll());
		return "homo";
	}
	/**
	 * 转入注册界面
	...
	public void userLogins( HttpServletRequest httpreq,HttpServletResponse resp) throws IOException{
		...
		if(remap.getCode()!=null&&remap.getCode()==200){ //成立则登陆成功
			request.addData("phone", loginUser);
	...
	public String userinfo(HttpServletRequest req){
		...
		r.addDataMap(SessionUtil.getUser(req));
		Results results= userService.findUserinfo(r);
		SessionUtil.saveUserInfo(req, results.getDataAll());
		return "userinfo";
	}
	/**
	 * 用户修改个人信息post请求
